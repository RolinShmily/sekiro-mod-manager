...
有两个组件以二进制形式再分发，其**许可证原文随每一次发布一并提供**，位于 [`licenses/`](licenses/)：

| 组件 | 许可证 | 原文 |
| :--- | :--- | :--- |
| Inter、JetBrains Mono、Noto Sans SC —— 内嵌于应用的子集化 `.woff2` 字体 | SIL Open Font License 1.1 | [`OFL-1.1.txt`](licenses/OFL-1.1.txt) |
| RARLAB UnRAR —— 经 `unrar_sys 0.5.8` 引入，静态链接进 `smm.exe` 与 `Sekiro-Mod-Manager.exe` | UnRAR freeware license（非 OSI） | [`UnRAR.txt`](licenses/UnRAR.txt) |
...
`licenses/` 中的两份文件均为上游原文。升级 `@fontsource/*` 或 `unrar_sys` 时请重新
